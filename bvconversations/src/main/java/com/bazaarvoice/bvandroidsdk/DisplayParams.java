...
/**
 * Bazaarvoice Conversations Display Request Parameters
 */
public class DisplayParams extends BazaarParams {
	...
	/**
	 * Boolean flag indicating whether to exclude content (reviews, questions, etc.) from other products in the same family as the requested product. This setting only affects any nested content that is returned. For example, {@code "&filter=productid:eq:1101&include=reviews&excludeFamily=true"} limits returned review content to just that of product 1101 and not any of the products in the same family. If a value is not defined, content on all products in the family is returned.
	 */
